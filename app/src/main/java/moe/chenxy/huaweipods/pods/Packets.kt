...
/**
 * ANC mode values for HUAWEI earphones (used in SET commands).
 * HUAWEI payload is 01 01 [value], except Adaptive which uses 01 01 00 08.
 */
object AncMode {
    const val OFF = 0x01
    const val NOISE_CANCELLATION = 0x02
    // ANC intensity payloads from HUAWEI captures: Smart/Light/Medium/Deep.
...
/**
 * Master EQ preset IDs for HUAWEI Enco X3 ("大师调音" / Master Tuning).
...
object Enums {
    ...
    val ANC_NOISE_CANCEL: ByteArray = HuaweiPackets.buildPacket(
    ...
    val ANC_NOISE_CANCEL_SMART: ByteArray = HuaweiPackets.buildPacket(
    ...
    val ANC_NOISE_CANCEL_LIGHT: ByteArray = HuaweiPackets.buildPacket(
    ...
    val ANC_NOISE_CANCEL_MEDIUM: ByteArray = HuaweiPackets.buildPacket(
    ...
    val ANC_NOISE_CANCEL_DEEP: ByteArray = HuaweiPackets.buildPacket(
    ...
    val ANC_TRANSPARENCY: ByteArray = HuaweiPackets.buildPacket(
    ...
    val TRANSPARENCY_VOCAL_ENHANCEMENT_ON: ByteArray = HuaweiPackets.buildPacket(
        cmd = Cmd.SET_ANC,
        seq = 0x57,
    ...
    val TRANSPARENCY_VOCAL_ENHANCEMENT_OFF: ByteArray = HuaweiPackets.buildPacket(
        cmd = Cmd.SET_ANC,
        seq = 0x57,
    ...
    val ANC_OFF: ByteArray = HuaweiPackets.buildPacket(
    ...
    val ANC_ADAPTIVE: ByteArray = HuaweiPackets.buildPacket(
    ...
    val QUERY_NOTIFICATION_SUPPORT: ByteArray = HuaweiPackets.buildPacket(
    ...
    fun registerMultiNotification(ids: ByteArray): ByteArray {
        ...
        return HuaweiPackets.buildPacket(cmd = Cmd.REGISTER_MULTI_NOTIFICATION, payload = payload)
    }

    /** Query ANC mode: AA 09 00 00 0C 01 00 02 00 01 01 */
    val QUERY_ANC: ByteArray = HuaweiPackets.buildPacket(
        cmd = Cmd.QUERY_ANC_MODE, payload = byteArrayOf(0x01, 0x01)
    )

    /** Enable game mode main switch: AA 09 00 00 03 04 00 02 00 28 01 */
    val GAME_MODE_ON: ByteArray = HuaweiPackets.buildPacket(
    ...
    val GAME_MODE_OFF: ByteArray = HuaweiPackets.buildPacket(
    ...
    val GAME_LOW_LATENCY_ON: ByteArray = HuaweiPackets.buildPacket(
    ...
    val GAME_LOW_LATENCY_OFF: ByteArray = HuaweiPackets.buildPacket(
    ...
    fun spatialAudioPacket(mode: Int): ByteArray = HuaweiPackets.buildPacket(
    ...
    fun eqPresetPacket(presetId: Int): ByteArray = HuaweiPackets.buildPacket(
        cmd = Cmd.SET_EQ,
        payload = byteArrayOf(presetId.toByte())
    )

    /** Query current EQ preset: AA 07 00 00 0F 01 F0 00 00 */
    val QUERY_EQ: ByteArray = HuaweiPackets.buildPacket(
        cmd = Cmd.QUERY_EQ_PRESET, payload = byteArrayOf()
    )

    /** Set spatial sound switch: AA 09 00 00 03 04 F0 02 00 1B [00/01]. */
    fun spatialSoundSwitchPacket(enabled: Boolean): ByteArray = HuaweiPackets.buildPacket(
    ...
    fun dualDeviceConnectionPacket(enabled: Boolean): ByteArray = HuaweiPackets.buildPacket(
        cmd = Cmd.SET_GAME_MODE,
...
/**
 * Parser for HUAWEI earphone battery response packets.
 *
...
/**
 * Parser for HUAWEI earphone ANC mode response/notification packets.
 *
 ...
 * HUAWEI buds, so subscribing the advertised list (minus the 0xFx debug channels)
