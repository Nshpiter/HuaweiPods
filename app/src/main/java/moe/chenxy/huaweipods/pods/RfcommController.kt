...
import moe.chenxy.huaweipods.utils.MediaControl
...
import moe.chenxy.huaweipods.utils.miuiStrongToast.MiuiStrongToastUtil.cancelPodsNotificationByMiuiBt
...
object RfcommController {
    private const val TAG = "HuaweiPods-RfcommController"
    ...
    private val HUAWEI_RFCOMM_UUID: UUID = UUID.fromString("0000079A-D102-11E1-9B23-00025B00A5A5")
    ...
    private fun changeUIAncStatus(status: Int) {
        ...
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_ANC_CHANGED) {
            if (::mDevice.isInitialized) this.putExtra("address", mDevice.address)
            this.putExtra("status", status)
        }
        sendExternalPodsStatusBroadcast(HuaweiPodsAction.ACTION_PODS_ANC_CHANGED) {
            putExtra("status", status)
        }
    }

    private fun changeUIBatteryStatus(status: BatteryParams) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_BATTERY_CHANGED) {
            if (::mDevice.isInitialized) this.putExtra("address", mDevice.address)
            this.putExtra("status", status)
            putBatteryExtras(status)
        }
        sendExternalPodsStatusBroadcast(HuaweiPodsAction.ACTION_PODS_BATTERY_CHANGED) {
            putExtra("status", status)
            putBatteryExtras(status)
        }
    }
    ...
    private fun changeUIWearStatus(status: WearStatus) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_WEAR_STATUS_CHANGED) {
            if (::mDevice.isInitialized) this.putExtra("address", mDevice.address)
            putWearStatusExtras(status)
        }
        sendExternalPodsStatusBroadcast(HuaweiPodsAction.ACTION_PODS_WEAR_STATUS_CHANGED) {
            putWearStatusExtras(status)
        }
    }

    private fun changeUIGameModeStatus(enabled: Boolean) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_GAME_MODE_CHANGED) {
            this.putExtra("enabled", enabled)
        }
    }

    private fun changeUITransparencyVocalEnhancementStatus(enabled: Boolean) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_TRANSPARENCY_VOCAL_ENHANCEMENT_CHANGED) {
            this.putExtra("enabled", enabled)
        }
        sendExternalPodsStatusBroadcast(HuaweiPodsAction.ACTION_PODS_TRANSPARENCY_VOCAL_ENHANCEMENT_CHANGED) {
            putExtra("enabled", enabled)
        }
    }

    private fun changeUISpatialAudioStatus(mode: Int) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_SPATIAL_AUDIO_CHANGED) {
            this.putExtra("mode", mode)
        }
    }

    private fun changeUIEqPreset(presetId: Int) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_EQ_PRESET_CHANGED) {
            this.putExtra("preset", presetId)
        }
    }

    private fun changeUISmartAncLevel(ordinal: Int) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_SMART_ANC_LEVEL_CHANGED) {
            this.putExtra("ordinal", ordinal)
        }
    }

    private fun changeUIDualDeviceConnectionStatus(enabled: Boolean) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_DUAL_DEVICE_CONNECTION_CHANGED) {
            this.putExtra("enabled", enabled)
        }
    }
        ...
        when (HuaweiPodsAction.canonical(intent.action)) {
            HuaweiPodsAction.ACTION_PODS_UI_INIT -> {
                ...
                if (::mDevice.isInitialized && isConnected) {
                    sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_CONNECTED) {
                        this.putExtra("address", mDevice.address)
                        this.putExtra("device_name", mDevice.name ?: cachedDeviceName)
                    }
                    sendExternalPodsStatusBroadcast(HuaweiPodsAction.ACTION_PODS_CONNECTED) {
            ...
            HuaweiPodsAction.ACTION_GAME_MODE_SET -> {
                val enabled = intent.getBooleanExtra("enabled", false)
                setGameMode(enabled)
            }
            HuaweiPodsAction.ACTION_AUTO_GAME_MODE_CHANGED -> {
            ...
            HuaweiPodsAction.ACTION_GAME_MODE_IMPLEMENTATION_CHANGED -> {
            ...
            HuaweiPodsAction.ACTION_RFCOMM_LOG_CONNECT -> {
                if (BuildConfig.DEBUG && !RfcommLog.isEnabled()) {
                    RfcommLog.setEnabled(true, mContext)
                }
            }
            HuaweiPodsAction.ACTION_RFCOMM_LOG_DISCONNECT -> {
                if (BuildConfig.DEBUG) RfcommLog.setEnabled(false)
            }
            HuaweiPodsAction.ACTION_RFCOMM_LOG_CLEAR -> {
                if (BuildConfig.DEBUG) RfcommLog.clear()
            }
            HuaweiPodsAction.ACTION_RFCOMM_DEBUG_SEND -> {
    ...
    private fun changeUIConnectionState(state: String) {
        sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_CONNECTION_STATE_CHANGED) {
    ...
    private fun miuiAncLevel(anc: Int, transparencyVocalEnhancement: Boolean): String {
        // MIUI level codes are not ordered like HUAWEI payloads:
    ...
    private fun createRfcommSocket(device: BluetoothDevice): BluetoothSocket {
        return device.createRfcommSocketToServiceRecord(HUAWEI_RFCOMM_UUID)
    }
    ...
    fun connectPod(context: Context, device: BluetoothDevice, prefs: SharedPreferences, appRequested: Boolean = false) {
        ...
        Log.d(TAG, "RFCOMM UUID initial: $HUAWEI_RFCOMM_UUID")

        if (!receiverRegistered) {
            context.registerReceiver(broadcastReceiver, IntentFilter().apply {
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_ANC_SELECT)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_PODS_UI_INIT)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_PODS_UI_CLOSED)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_REFRESH_STATUS)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_GAME_MODE_SET)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_AUTO_GAME_MODE_CHANGED)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_GAME_MODE_IMPLEMENTATION_CHANGED)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_TRANSPARENCY_VOCAL_ENHANCEMENT_SET)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_SPATIAL_AUDIO_SET)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_EQ_PRESET_SET)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_DUAL_DEVICE_CONNECTION_SET)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_CYCLE_ANC)
                this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_CONFIG_CHANGED)
                if (BuildConfig.DEBUG) {
                    this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_RFCOMM_LOG_CONNECT)
                    this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_RFCOMM_LOG_DISCONNECT)
                    this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_RFCOMM_LOG_CLEAR)
                    this.addHuaweiPodsAction(HuaweiPodsAction.ACTION_RFCOMM_DEBUG_SEND)
                }
    ...
    private fun connectRfcomm(initialDelayMs: Long = 0L) {
        ...
        connectionJob = CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                Log.d(TAG, "RFCOMM connected! uuid=$HUAWEI_RFCOMM_UUID")
                RfcommLog.i(mContext, TAG, "connected uuid=$HUAWEI_RFCOMM_UUID")
                changeUIConnectionState("connecting")
                ...
                // advertised list (minus 0xFx debug channels) in handleHuaweiPacket.
    ...
    @OptIn(ExperimentalStdlibApi::class)
    private fun handleHuaweiPacket(packet: ByteArray) {
        ...
        Log.d(TAG, "Unknown HUAWEI packet: ${packet.toHexString(HexFormat.UpperCase)}")
    }
    ...
    fun disconnectedPod(context: Context, device: BluetoothDevice) {
        ...
        mContext?.let {
            ...
            sendAppStatusBroadcast(HuaweiPodsAction.ACTION_PODS_DISCONNECTED) {
