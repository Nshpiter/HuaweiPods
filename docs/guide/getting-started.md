...
首次启用模块，或从 API 101 版本升级到 1.8.0 后，如果 LSPosed 提示重启作用域，请执行一次。之后覆盖安装同签名的新版本时，模块会优先自动热重载；只有当前进程不具备安全恢复条件时才会回退为重启提示。完成后连接已配对的受支持设备。
...
- 1.8.0 及以上同签名版本会优先自动热重载；从旧版首次升级或恢复条件不足时，按 LSPosed 提示重启一次作用域；
